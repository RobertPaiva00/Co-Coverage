...
bool leader_odom_arrived = false, follower_odom_arrived = false, role_changed = false;
struct leader_robot
{
	double xPos;
	double yPos;
...
int main(int argc, char** argv){
	...
	while(ros::ok()){
		...
		follower_bpgt_sub = node.subscribe<nav_msgs::Odometry>(INstampname + "/amcl_pose", 1, &odom_followerCallback);
		...
		leader_bpgt_sub = node.subscribe<nav_msgs::Odometry>(INleaderstampname + "/amcl_pose", 1, &odom_leaderCallback);
