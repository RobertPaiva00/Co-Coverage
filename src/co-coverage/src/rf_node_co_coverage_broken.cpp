...
void scanReceived(const sensor_msgs::LaserScan::ConstPtr& pt)
{
	...
	// 		goTo.update(target_inli, target_inthetai, target_invj);
	// 		goTo.upforces();
	...
	// 		// update repulsive forces
	// 		repulsive.upforces(goTo.li, goTo.thetai);

	// 		// update follower command values
	// 		follower.updatecmd(goTo.vid, goTo.fangular, repulsive.vid, repulsive.fangular, repulsive.U_positive);
...
// void initCoverage(){
// 	if (follower_odom_arrived){
// 		if (follower_odom.xPos < (x_goal + IDstamp + 0.1) && follower_odom.xPos > (x_goal + IDstamp - 0.1) && follower_odom.yPos < (y_goal + 0.1) && follower_odom.yPos > (y_goal - 0.1)){
// 			goToPoint = false;
// 		}
// 	}
// }
